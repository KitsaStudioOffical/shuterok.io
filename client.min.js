...
function setupSocketEvents() {
    ...
    socket.on('u', pack => {
        pack.p.forEach(srv => {
            ...
            if(!p) p = players[srv.id] = { ...srv, body: new SoftBody2D(srv.r, srv.col) };
            else {
...
class SoftBody2D {
    constructor(radius, color) {
        ...
        for (let i = 0; i < this.segments; i++) {
            ...
            this.nodes.push({ x, y, px: x, py: y, ax: 0, ay: 0, mass: 1 });
        }

        // Center node (controls movement)
        this.center = { x: 0, y: 0, px: 0, py: 0, ax: 0, ay: 0, mass: 1.5 };
        this.nodes.push(this.center);
        this.centerIndex = this.nodes.length - 1;

        // Structural springs + radial springs
        for (let i = 0; i < this.segments; i++) {
            let j = (i + 1) % this.segments;
            this.connect(i, j, 0.9); // ring
            this.connect(i, this.centerIndex, 0.2); // radial
            this.connect(i, (i + 2) % this.segments, 0.35); // cross stabilize
        }
    }
    ...
    applyPhysics(dt) {
        ...
        for (let n of this.nodes) {
            let nx = n.x + (n.x - n.px) * DAMP + n.ax * dt * dt;
            let ny = n.y + (n.y - n.py) * DAMP + (n.ay + GRAV) * dt * dt;
            n.px = n.x;
            n.py = n.y;
            n.x = nx;
            n.y = ny;
        ...
        for (let s = 0; s < 6; s++) {
            for (let sp of this.springs) {
                ...
                A.x += dx * force * (1 / A.mass);
                A.y += dy * force * (1 / A.mass);
                B.x -= dx * force * (1 / B.mass);
                B.y -= dy * force * (1 / B.mass);
            }
        }
    }

    // External movement input from your game (WASD or joystick)
    moveTo(worldX, worldY) {
        this.center.x = worldX;
        this.center.y = worldY;
    }

    update(dt, angle, move) {
        // Slight push to outer nodes for squash/stretch
        if (move) {
            for (let i = 0; i < this.segments; i++) {
                const n = this.nodes[i];
                n.ax += Math.cos(angle) * 0.02;
                n.ay += Math.sin(angle) * 0.02;
            }
        }
    ...
    draw(ctx, worldX, worldY, angle) {
        ...
        ctx.beginPath();
        for (let i = 0; i <= this.segments; i++) {
            let n = this.nodes[i % this.segments];
            let nx = this.nodes[(i + 1) % this.segments];

            let mx = (n.x + nx.x) * 0.5;
            let my = (n.y + nx.y) * 0.5;

            if (i === 0) ctx.moveTo(mx, my);
            else ctx.quadraticCurveTo(n.x, n.y, mx, my);
        }
        ctx.fill();

        // Eyes (same as your original SoftBody)
        ctx.fillStyle = "white";
        let off = this.r * 0.6;
        let sz = this.r * 0.25;

        let ex1 = Math.cos(angle - 0.6) * off;
        let ey1 = Math.sin(angle - 0.6) * off;
        let ex2 = Math.cos(angle + 0.6) * off;
        let ey2 = Math.sin(angle + 0.6) * off;

        ctx.beginPath();
        ctx.arc(ex1, ey1, sz, 0, Math.PI * 2);
        ctx.arc(ex2, ey2, sz, 0, Math.PI * 2);
        ctx.fill();

        ctx.fillStyle = "black";
        let lx = Math.cos(angle) * sz * 0.4;
        let ly = Math.sin(angle) * sz * 0.4;

        ctx.beginPath();
        ctx.arc(ex1 + lx, ey1 + ly, sz * 0.5, 0, Math.PI * 2);
        ctx.arc(ex2 + lx, ey2 + ly, sz * 0.5, 0, Math.PI * 2);
        ctx.fill();
